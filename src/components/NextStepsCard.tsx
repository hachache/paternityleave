...
export function NextStepsCard({
  ...
  return (
    <ol className="relative space-y-0">
      <div className="absolute left-[17px] sm:left-[19px] top-4 bottom-4 w-0.5 bg-slate-100" />
      {checklist.map(item => (
        <li key={item.label} className="group relative pl-11 sm:pl-12 py-3 sm:py-4 first:pt-0 last:pb-0">
          <span
            className={`absolute left-0 top-3 sm:top-4 inline-flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-xl text-sm font-bold shadow-sm ring-4 ring-white transition-colors duration-200 ${
              item.status === 'done'
          ...
          <div
            className={`space-y-1 p-3.5 sm:p-4 rounded-2xl transition-colors duration-200 ${
              item.status === 'active'
                ? 'bg-white border border-brand-100 shadow-soft'
